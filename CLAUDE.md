...
**Local dev database:** `.env.development.local` (gitignored) overrides `SECRET_MONGO_URI` to `mongodb://127.0.0.1:27017/sma-dev` in dev mode only, so `pnpm dev` writes to a local MongoDB instead of the production Atlas cluster. That Mongo runs as the Docker container `sma-dev-mongo` (image `mongo:8`, bound to 127.0.0.1, data in the `sma-dev-mongo-data` volume) — `docker start sma-dev-mongo` if it's stopped. Builds and `pnpm test:integration` run in production mode and do NOT pick this file up. One localStorage gotcha when switching databases: identities persist in the browser but their `Listener` docs don't exist in a fresh DB, so pages fail with "Public key not found" until you create/load a new identity on `/i`. And one schema gotcha: the `mongoose.models.X || mongoose.model(...)` HMR pattern means a schema edit does NOT take effect on Vite hot reload — the already-compiled model keeps the old field list and strict mode silently drops writes to new fields. Restart `pnpm dev` after any change to `src/models/*`. Split-deploy only: `CORS_ORIGINS` (comma-separated allowlist for cross-origin `/api` calls) and `PUBLIC_API_BASE` (backend origin the frontend should call; empty means same-origin).
...
### Per-room abuse limits (owner-set, mostly server-enforced)

Four knobs on `Listener`, all written through one signed endpoint (`PATCH /api/limits`, action `limits:set`, partial updates — each settings control sends only its own field) and read publicly (`GET /api/limits?rid=`) by the send page. Rules and the send gate live in `src/lib/server/roomLimits.ts`, unit-tested without a DB; UI is `LimitsSettings.svelte` in the Settings modal, except the images toggle, which renders as `ImagesToggle.svelte` beside `VoiceToggle` in a two-column row (it still writes through the same `limits:set` action and the shared `IRoomLimits` state from `$lib/types`). Defaults are chosen so every pre-existing room behaves exactly as before:

- `paused` (default `false`) — hard server 403 on every send. Blocks **sends only**: edits of already-sent messages and owner replies still work, deliberately — pause stops the inbound flood, it doesn't freeze existing threads.
- `imagesEnabled` (default `true`) — hard server 403 on a send carrying `imageData`. Default **on**, the opposite of `voiceEnabled`, because images predate their toggle; the gate therefore only acts on an explicit `false` (`imagesEnabled === false`), while voice stays opt-in (`voiceEnabled !== true` refuses audio — that check moved from inline in `/api/pgp` into the shared gate).
- `maxMessageLength` (0 = off, else 1–1000) — enforced **exactly client-side** (composer + edit textarea `maxlength` and counters) and **approximately server-side**: the server never sees plaintext, so it can't count characters, but it can bound the armored ciphertext at a size no compliant message could reach (`ciphertextCapFor` = 2600 + 6·N, covering worst-case UTF-8 + base64 + armor overhead, pinned empirically against real openpgp output in `roomLimits.test.ts`). The cap catches 16KB-when-you-asked-for-280 abuse, not the 281st character. Applies to edits via `/api/sent` too, else the limit is a dead letter (send 10 chars, edit in the essay).
- `rateLimitCount` + `rateLimitPeriod` (count 0 = off, else 1–10000 per `minute`|`hour`|`day`; the UI is a free number input plus a period toggle) — hard server 429, via an in-memory fixed window keyed by the **recipient** rid (`checkRoomRate`). Keying on the room, not the sender, is the point: sender identities are free to mint (Sybil), so per-sender limits are worthless, but the room being flooded is scarce and server-visible. The period is part of the window key, so switching periods starts a fresh window; an unrecognized stored period falls back to hourly rather than failing open. Period validation uses `Object.hasOwn`, not `in` — `'toString' in RATE_PERIODS` is true via the prototype chain. Checked **last**, after every other gate — consulting the window counts the send, and a send rejected for other reasons shouldn't eat the room's budget. Same single-process caveat as the per-IP limiter; a restart forgets the window, acceptable for an abuse cap.

The whole gate runs before anything is written, which also fixed an older quirk: a send to a nonexistent room used to create an orphan `Message` doc before discovering the listener was missing; now it 404s first.
